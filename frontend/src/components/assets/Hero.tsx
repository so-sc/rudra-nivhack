import React from "react"

function Hero() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      width="940"
      height="647"
      overflow="visible"
      viewBox="0 0 940 647"
    >
      <defs>
        <linearGradient
          id="hero-illustration-a"
          x1="100%"
          x2="0%"
          y1="0%"
          y2="100%"
        >
          <stop offset="0%" stopColor="#261FB6"></stop>
          <stop offset="100%" stopColor="#4950F6"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-b"
          x1="89.764%"
          x2="11.987%"
          y1="16.809%"
          y2="82.178%"
        >
          <stop offset="0%" stopColor="#FC8464"></stop>
          <stop offset="100%" stopColor="#FB5C32"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-c"
          x1="100%"
          x2="4.439%"
          y1="23.096%"
          y2="96.586%"
        >
          <stop offset="0%" stopColor="#1ADAB7"></stop>
          <stop offset="100%" stopColor="#55EBD0"></stop>
        </linearGradient>
        <filter
          id="hero-illustration-d"
          width="126.1%"
          height="400%"
          x="-13%"
          y="-150%"
          filterUnits="objectBoundingBox"
        >
          <feGaussianBlur in="SourceGraphic" stdDeviation="32"></feGaussianBlur>
        </filter>
        <linearGradient
          id="hero-illustration-f"
          x1="0%"
          x2="46.694%"
          y1="100%"
          y2="42.915%"
        >
          <stop offset="0%" stopColor="#EEF1FA"></stop>
          <stop offset="100%" stopColor="#FFF"></stop>
        </linearGradient>
        <rect id="hero-illustration-e" width="800" height="450" rx="4"></rect>
        <linearGradient
          id="hero-illustration-g"
          x1="100%"
          x2="0%"
          y1="-12.816%"
          y2="-12.816%"
        >
          <stop offset="0%" stopColor="#D2DAF0" stopOpacity="0"></stop>
          <stop offset="50.045%" stopColor="#D2DAF0"></stop>
          <stop offset="100%" stopColor="#D2DAF0" stopOpacity="0"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-h"
          x1="116.514%"
          x2="0%"
          y1="-21.201%"
          y2="100%"
        >
          <stop offset="0%" stopColor="#55EBD0"></stop>
          <stop offset="100%" stopColor="#4950F6"></stop>
        </linearGradient>
        <path id="hero-illustration-j" d="M0 0h308v288H0z"></path>
        <filter
          id="hero-illustration-i"
          width="139%"
          height="141.7%"
          x="-15.6%"
          y="-12.5%"
          filterUnits="objectBoundingBox"
        >
          <feOffset
            dx="12"
            dy="24"
            in="SourceAlpha"
            result="shadowOffsetOuter1"
          ></feOffset>
          <feGaussianBlur
            in="shadowOffsetOuter1"
            result="shadowBlurOuter1"
            stdDeviation="16"
          ></feGaussianBlur>
          <feColorMatrix
            in="shadowBlurOuter1"
            values="0 0 0 0 0.0666666667 0 0 0 0 0.062745098 0 0 0 0 0.243137255 0 0 0 0.08 0"
          ></feColorMatrix>
        </filter>
        <circle id="hero-illustration-k" cx="16" cy="16" r="16"></circle>
        <linearGradient
          id="hero-illustration-m"
          x1="50%"
          x2="50%"
          y1="0%"
          y2="100%"
        >
          <stop offset="0%" stopColor="#C6FDF3"></stop>
          <stop offset="100%" stopColor="#C6FDF3" stopOpacity="0"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-n"
          x1="2.875%"
          x2="99.412%"
          y1="89.028%"
          y2="6.885%"
        >
          <stop offset="0%" stopColor="#83F0DD"></stop>
          <stop offset="50.924%" stopColor="#1ADAB7"></stop>
          <stop offset="100%" stopColor="#55EBD0"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-o"
          x1="50%"
          x2="50%"
          y1="1.569%"
          y2="97.315%"
        >
          <stop offset="0%" stopColor="#FF97AA"></stop>
          <stop offset="100%" stopColor="#FF6381"></stop>
        </linearGradient>
        <linearGradient
          id="hero-illustration-p"
          x1="50%"
          x2="50%"
          y1="0%"
          y2="100%"
        >
          <stop offset="0%" stopColor="#FCAC96"></stop>
          <stop offset="100%" stopColor="#FC8464"></stop>
        </linearGradient>
        <circle id="hero-illustration-r" cx="28" cy="28" r="28"></circle>
        <filter
          id="hero-illustration-q"
          width="314.3%"
          height="314.3%"
          x="-85.7%"
          y="-64.3%"
          filterUnits="objectBoundingBox"
        >
          <feOffset
            dx="12"
            dy="24"
            in="SourceAlpha"
            result="shadowOffsetOuter1"
          ></feOffset>
          <feGaussianBlur
            in="shadowOffsetOuter1"
            result="shadowBlurOuter1"
            stdDeviation="16"
          ></feGaussianBlur>
          <feColorMatrix
            in="shadowBlurOuter1"
            values="0 0 0 0 0.0666666667 0 0 0 0 0.062745098 0 0 0 0 0.243137255 0 0 0 0.08 0"
          ></feColorMatrix>
        </filter>
        <circle id="hero-illustration-t" cx="36" cy="36" r="36"></circle>
        <filter
          id="hero-illustration-s"
          width="266.7%"
          height="266.7%"
          x="-66.7%"
          y="-50%"
          filterUnits="objectBoundingBox"
        >
          <feOffset
            dx="12"
            dy="24"
            in="SourceAlpha"
            result="shadowOffsetOuter1"
          ></feOffset>
          <feGaussianBlur
            in="shadowOffsetOuter1"
            result="shadowBlurOuter1"
            stdDeviation="16"
          ></feGaussianBlur>
          <feColorMatrix
            in="shadowBlurOuter1"
            values="0 0 0 0 0.0666666667 0 0 0 0 0.062745098 0 0 0 0 0.243137255 0 0 0 0.08 0"
          ></feColorMatrix>
        </filter>
        <circle id="hero-illustration-v" cx="28" cy="28" r="28"></circle>
        <filter
          id="hero-illustration-u"
          width="314.3%"
          height="314.3%"
          x="-85.7%"
          y="-64.3%"
          filterUnits="objectBoundingBox"
        >
          <feOffset
            dx="12"
            dy="24"
            in="SourceAlpha"
            result="shadowOffsetOuter1"
          ></feOffset>
          <feGaussianBlur
            in="shadowOffsetOuter1"
            result="shadowBlurOuter1"
            stdDeviation="16"
          ></feGaussianBlur>
          <feColorMatrix
            in="shadowBlurOuter1"
            values="0 0 0 0 0.0666666667 0 0 0 0 0.062745098 0 0 0 0 0.243137255 0 0 0 0.08 0"
          ></feColorMatrix>
        </filter>
        <circle id="hero-illustration-x" cx="24" cy="24" r="24"></circle>
        <filter
          id="hero-illustration-w"
          width="350%"
          height="350%"
          x="-100%"
          y="-75%"
          filterUnits="objectBoundingBox"
        >
          <feOffset
            dx="12"
            dy="24"
            in="SourceAlpha"
            result="shadowOffsetOuter1"
          ></feOffset>
          <feGaussianBlur
            in="shadowOffsetOuter1"
            result="shadowBlurOuter1"
            stdDeviation="16"
          ></feGaussianBlur>
          <feColorMatrix
            in="shadowBlurOuter1"
            values="0 0 0 0 0.0666666667 0 0 0 0 0.062745098 0 0 0 0 0.243137255 0 0 0 0.08 0"
          ></feColorMatrix>
        </filter>
      </defs>
      <g fill="none" fillRule="evenodd">
        <path
          fill="url(#hero-illustration-a)"
          d="M600 300c0 165.685-134.315 300-300 300S0 465.685 0 300 134.315 0 300 0s300 134.315 300 300z"
          className="anime-element stroke-animation"
        ></path>
        <g transform="translate(435 518)">
          <circle
            cx="106"
            cy="32"
            r="32"
            fill="url(#hero-illustration-b)"
            className="anime-element fadeup-animation"
          ></circle>
          <circle
            cx="12"
            cy="117"
            r="12"
            fill="url(#hero-illustration-c)"
            className="anime-element fadeup-animation"
          ></circle>
        </g>
        <g className="anime-element fadeleft-animation">
          <g transform="translate(103 75)">
            <path
              fill="#11103E"
              fillOpacity="0.24"
              d="M32 410h736v64H32z"
              filter="url(#hero-illustration-d)"
            ></path>
            <use
              fill="url(#hero-illustration-f)"
              xlinkHref="#hero-illustration-e"
            ></use>
          </g>
          <g transform="translate(123 87)">
            <circle cx="4" cy="4" r="4" fill="#D2DAF0"></circle>
            <circle cx="20" cy="4" r="4" fill="#D2DAF0"></circle>
            <circle cx="36" cy="4" r="4" fill="#D2DAF0"></circle>
            <path fill="#FFF" fillOpacity="0.56" d="M736 2h24v4h-24z"></path>
            <path fill="url(#hero-illustration-g)" d="M27 20h706v2H27z"></path>
          </g>
          <g transform="translate(396 157)">
            <path fill="#FFF" d="M0 0h308v144H0z"></path>
            <path
              fill="#EEF1FA"
              d="M28 119h252v1H28zm0-25h252v1H28zm0-25h252v1H28zm0-25h252v1H28z"
            ></path>
            <path
              stroke="url(#hero-illustration-h)"
              strokeWidth="3"
              d="M26 119l49.19-38.316 14.615 19.035 16.36-19.035 31.306 4.272 33.079-23.54 34.86 38.303 15.625-50.412 16.356 12.109 16.472-12.11 15.075 21.443 16.976-21.442"
            ></path>
            <rect
              width="24"
              height="4"
              x="28"
              y="20"
              fill="#D2DAF0"
              rx="2"
            ></rect>
          </g>
          <path fill="#FFF" d="M396 333h308v144H396z"></path>
          <path
            fill="#EEF1FA"
            d="M424 452h252v1H424zm0-25h252v1H424zm0-25h252v1H424zm0-25h252v1H424z"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M28 24h12v96H28z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-p)"
            d="M52 67h12v53H52z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M76 24h12v96H76z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-p)"
            d="M100 82h12v38h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M124 45h12v75h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-p)"
            d="M148 67h12v53h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M172 82h12v38h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-p)"
            d="M196 45h12v75h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M220 67h12v53h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-p)"
            d="M244 82h12v38h-12z"
            transform="translate(396 333)"
          ></path>
          <path
            fill="url(#hero-illustration-o)"
            d="M268 45h12v75h-12z"
            transform="translate(396 333)"
          ></path>
        </g>
        <g transform="translate(56 157)" className="0">
          <use
            fill="#000"
            filter="url(#hero-illustration-i)"
            xlinkHref="#hero-illustration-j"
          ></use>
          <use fill="#FFF" xlinkHref="#hero-illustration-j"></use>
          <path fill="#EEF1FA" d="M0 191h308v1H0zM28 44h252v1H28z"></path>
          <rect
            width="24"
            height="4"
            x="28"
            y="20"
            fill="#D2DAF0"
            rx="2"
          ></rect>
          <rect
            width="64"
            height="4"
            x="28"
            y="216"
            fill="#ABABC9"
            rx="2"
          ></rect>
          <rect
            width="24"
            height="4"
            x="256"
            y="216"
            fill="#D2DAF0"
            rx="2"
          ></rect>
          <rect
            width="24"
            height="4"
            x="64"
            y="20"
            fill="#D2DAF0"
            rx="2"
          ></rect>
          <g transform="translate(28 232)">
            <mask id="hero-illustration-l" fill="#fff">
              <use xlinkHref="#hero-illustration-k"></use>
            </mask>
            <use fill="#ABABC9" xlinkHref="#hero-illustration-k"></use>
          </g>
          <path
            fill="url(#hero-illustration-m)"
            d="M308 89.82v101.063H0v-33.728l3.038-3.799 10.783-7.894 8.687-2.646h9.246l21.198 10.54 9.852 3.251 6.608-3.25 8.644-12.005 6.87-6.388 7.892-1.512 10.906 6.066 13.895 15.802 11.188 5.944h5.692l7.314-3.28 7.19-9.37 5.879-13.65 10.774-21.657 8.34-14.026 9.243-12.336 9.594-7.043 15.407-3.881 6.362 1.497 14.939 7.765 10.531 8.166 13.17 12.338 12.362 7.52 12.756 3.746 8.366-1.542 8.96-5.366 10.183-15.377L308 89.82z"
          ></path>
          <path
            fill="url(#hero-illustration-n)"
            fillRule="nonzero"
            d="M308 84.066v8.885c-13.215 32.373-36.292 33.308-68.007 2.699-18.456-17.814-33.74-21.583-47.058-13.978-10.675 6.097-19.593 18.713-29.342 38.963-2.594 5.388-10.386 22.73-11.136 24.336-11.298 24.181-27.067 24.181-45.624.363-12.17-13.657-21.486-13.657-29.221-.248-3.596 6.232-6.872 9.923-10.352 11.627-3.677 1.8-7.4 1.412-12.305-.82-2.416-1.098-12.51-6.8-14.82-7.981a101.498 101.498 0 00-4.586-2.208c-11.912-5.359-23.695-.926-35.549 13.65v-4.62c11.94-13.38 24.255-17.4 36.78-11.766 1.571.707 3.13 1.46 4.72 2.273 2.416 1.235 12.425 6.889 14.697 7.922 4.167 1.895 7.013 2.192 9.744.855 2.836-1.388 5.748-4.67 9.072-10.432 8.843-15.326 20.622-15.326 34.123-.172 17.344 22.257 30.338 22.257 40.603.287.734-1.572 8.536-18.936 11.151-24.368 9.998-20.768 19.193-33.776 30.557-40.266 14.618-8.348 31.298-4.234 50.63 14.424 32.601 31.465 54.068 28.367 65.923-9.425z"
          ></path>
        </g>
        <g className="anime-element fadeleft-animation">
          <g transform="translate(164 483)">
            <use
              fill="#000"
              filter="url(#hero-illustration-q)"
              xlinkHref="#hero-illustration-r"
            ></use>
            <use fill="#FFF" xlinkHref="#hero-illustration-r"></use>
          </g>
          <path
            fill="#1DA1F2"
            fillRule="nonzero"
            d="M200 506c-.6.3-1.2.4-1.9.5.7-.4 1.2-1 1.4-1.8-.6.4-1.3.6-2.1.8-.6-.6-1.5-1-2.4-1-1.7 0-3.2 1.5-3.2 3.3 0 .3 0 .5.1.7-2.7-.1-5.2-1.4-6.8-3.4-.3.5-.4 1-.4 1.7 0 1.1.6 2.1 1.5 2.7-.5 0-1-.2-1.5-.4 0 1.6 1.1 2.9 2.6 3.2-.3.1-.6.1-.9.1-.2 0-.4 0-.6-.1.4 1.3 1.6 2.3 3.1 2.3-1.1.9-2.5 1.4-4.1 1.4h-.8c1.5.9 3.2 1.5 5 1.5 6 0 9.3-5 9.3-9.3v-.4c.7-.5 1.3-1.1 1.7-1.8z"
          ></path>
        </g>
        <g className="anime-element fadeleft-animation">
          <g transform="translate(251 390)">
            <use
              fill="#000"
              filter="url(#hero-illustration-s)"
              xlinkHref="#hero-illustration-t"
            ></use>
            <use fill="#FFF" xlinkHref="#hero-illustration-t"></use>
          </g>
          <g fillRule="nonzero">
            <path
              fill="#DFA22F"
              d="M290.56 415.485a2.137 2.137 0 10-4.066 1.32l5.526 17.004a2.137 2.137 0 002.6 1.325c1.135-.327 1.826-1.532 1.464-2.646l-5.525-17.003"
            ></path>
            <path
              fill="#3CB187"
              d="M281.996 418.267a2.137 2.137 0 00-4.065 1.321l5.526 17.003a2.137 2.137 0 002.6 1.325c1.134-.326 1.826-1.531 1.464-2.645l-5.525-17.004"
            ></path>
            <path
              fill="#CE1E5B"
              d="M297.515 429.567a2.137 2.137 0 10-1.32-4.065l-17.004 5.526a2.137 2.137 0 00-1.325 2.6c.327 1.134 1.532 1.825 2.646 1.464l17.003-5.525"
            ></path>
            <path
              fill="#392538"
              d="M282.735 434.37l4.064-1.321-1.32-4.065-4.065 1.321 1.321 4.064"
            ></path>
            <path
              fill="#BB242A"
              d="M291.298 431.587l4.064-1.32-1.32-4.066-4.065 1.321 1.321 4.065"
            ></path>
            <path
              fill="#72C5CD"
              d="M294.733 421.004a2.137 2.137 0 00-1.321-4.066l-17.003 5.527a2.136 2.136 0 00-1.325 2.6c.326 1.134 1.531 1.825 2.645 1.463l17.004-5.524"
            ></path>
            <path
              fill="#248C73"
              d="M279.952 425.806l4.065-1.32c-.5-1.537-.964-2.965-1.32-4.065l-4.066 1.322 1.321 4.063"
            ></path>
            <path
              fill="#62803A"
              d="M288.515 423.024l4.065-1.32-1.321-4.066-4.065 1.321 1.321 4.065"
            ></path>
          </g>
        </g>
        <g className="anime-element fadeleft-animation">
          <g transform="translate(28 221)">
            <use
              fill="#000"
              filter="url(#hero-illustration-u)"
              xlinkHref="#hero-illustration-v"
            ></use>
            <use fill="#FFF" xlinkHref="#hero-illustration-v"></use>
          </g>
          <text
            fill="#11103E"
            fontFamily="AppleColorEmoji, Apple Color Emoji"
            fontSize="19"
            transform="translate(28 221)"
          >
            <tspan x="17" y="36">
              🤗
            </tspan>
          </text>
        </g>
        <g className="anime-element fadeleft-animation">
          <g transform="translate(325 257)">
            <use
              fill="#000"
              filter="url(#hero-illustration-w)"
              xlinkHref="#hero-illustration-x"
            ></use>
            <use fill="#FFF" xlinkHref="#hero-illustration-x"></use>
          </g>
          <text
            fill="#11103E"
            fontFamily="AppleColorEmoji, Apple Color Emoji"
            fontSize="16"
            transform="translate(325 257)"
          >
            <tspan x="13" y="31">
              👋
            </tspan>
          </text>
        </g>
      </g>
    </svg>
  )
}

export default Hero
